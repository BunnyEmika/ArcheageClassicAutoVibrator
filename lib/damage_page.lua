...
function M.Open(reloadFn)
	wnd = api.Interface:CreateWindow("aav-pop", "Damage Settings")
    ...
    intValuelbl:SetText(tostring(settings.maxIntensityDamage))
    ...
    intIncreaseBtn:SetHandler("OnClick", function(_, button) 
        if api.Input:IsShiftKeyDown() then
            settings.maxIntensityDamage = (settings.maxIntensityDamage + 1000)
        else
            settings.maxIntensityDamage = (settings.maxIntensityDamage + 100)
        end
        intValuelbl:SetText(tostring(settings.maxIntensityDamage)) 
    end)
    ...
    intDecreaseBtn:SetHandler("OnClick", function(_, button) 
        ...
        if settings.maxIntensityDamage < 0 then settings.maxIntensityDamage = 0 end
        intValuelbl:SetText(tostring(settings.maxIntensityDamage)) 
    end)
    ...
    durValuelbl:SetText(tostring(settings.maxDurationDamage))
    ...
    durIncreaseBtn:SetHandler("OnClick", function(_, button) 
        if api.Input:IsShiftKeyDown() then
            settings.maxDurationDamage = (settings.maxDurationDamage + 1000)
        else
            settings.maxDurationDamage = (settings.maxDurationDamage + 100)
        end
        durValuelbl:SetText(tostring(settings.maxDurationDamage)) 
    end)
    ...
    durDecreaseBtn:SetHandler("OnClick", function(_, button) 
        ...
        if settings.maxDurationDamage < 0 then settings.maxDurationDamage = 0 end
        durValuelbl:SetText(tostring(settings.maxDurationDamage)) 
    end)

    -- Max Duration
    local maxDurationlbl = wnd:CreateChildWidget("label", "maxDurationlbl", 0, true)
	maxDurationlbl:SetText("Max duration caused by damage in Ms")
    ...
    maxDurValuelbl:SetText(tostring(settings.maxDamageDurationMs))
    ...
    maxDurIncreaseBtn:SetHandler("OnClick", function(_, button) 
        if api.Input:IsShiftKeyDown() then
            settings.maxDamageDurationMs = (settings.maxDamageDurationMs + 1000)
        else
            settings.maxDamageDurationMs = (settings.maxDamageDurationMs + 100)
        end
        maxDurValuelbl:SetText(tostring(settings.maxDamageDurationMs)) 
    end)
    ...
    maxDurDecreaseBtn:SetHandler("OnClick", function(_, button) 
        ...
        if settings.maxDamageDurationMs < 0 then settings.maxDamageDurationMs = 0 end
        maxDurValuelbl:SetText(tostring(settings.maxDamageDurationMs)) 
    end)
	...
	modOutlbl:SetText("Outgoing Damage Multiplier")
    ...
    modOutValuelbl:SetText(tostring(settings.damageOutMod))
    ...
    modOutIncreaseBtn:SetHandler("OnClick", function(_, button) 
        if api.Input:IsShiftKeyDown() then
            settings.damageOutMod = (settings.damageOutMod + 0.1)
        else
            settings.damageOutMod = (settings.damageOutMod + 0.01)
        end
        modOutValuelbl:SetText(tostring(settings.damageOutMod)) 
    end)
    ...
    modOutDecreaseBtn:SetHandler("OnClick", function(_, button) 
        ...
        if settings.damageOutMod < 0.01 then settings.damageOutMod = 0 end
        modOutValuelbl:SetText(tostring(settings.damageOutMod)) 
    end)

    -- Incoming Mod
    local modInlbl = wnd:CreateChildWidget("label", "modInlbl", 0, true)
	modInlbl:SetText("Incoming Damage Multiplier")
    ...
    modInValuelbl:SetText(tostring(settings.damageInMod))
    ...
    modInIncreaseBtn:SetHandler("OnClick", function(_, button) 
        if api.Input:IsShiftKeyDown() then
            settings.damageInMod = (settings.damageInMod + 0.1)
        else
            settings.damageInMod = (settings.damageInMod + 0.01)
        end
        modInValuelbl:SetText(tostring(settings.damageInMod)) 
    end)
    ...
    modInDecreaseBtn:SetHandler("OnClick", function(_, button) 
        ...
        if settings.damageInMod < 0.01 then settings.damageInMod = 0 end
        modInValuelbl:SetText(tostring(settings.damageInMod)) 
    end)
