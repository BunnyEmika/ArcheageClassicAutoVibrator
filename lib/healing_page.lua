...
function M.Open(reloadFn)
	wnd = api.Interface:CreateWindow("aav-pop", "Healing Settings")
    ...
    intValuelbl:SetText(tostring(settings.maxIntensityHealing))
    ...
    intIncreaseBtn:SetHandler("OnClick", function(_, button) 
        if api.Input:IsShiftKeyDown() then
            settings.maxIntensityHealing = (settings.maxIntensityHealing + 1000)
        else
            settings.maxIntensityHealing = (settings.maxIntensityHealing + 100)
        end
        intValuelbl:SetText(tostring(settings.maxIntensityHealing)) 
    end)
    ...
    intDecreaseBtn:SetHandler("OnClick", function(_, button) 
        ...
        if settings.maxIntensityHealing < 0 then settings.maxIntensityHealing = 0 end
        intValuelbl:SetText(tostring(settings.maxIntensityHealing)) 
    end)
    ...
    durValuelbl:SetText(tostring(settings.maxDurationHealing))
    ...
    durIncreaseBtn:SetHandler("OnClick", function(_, button) 
        if api.Input:IsShiftKeyDown() then
            settings.maxDurationHealing = (settings.maxDurationHealing + 1000)
        else
            settings.maxDurationHealing = (settings.maxDurationHealing + 100)
        end
        durValuelbl:SetText(tostring(settings.maxDurationHealing)) 
    end)
    ...
    durDecreaseBtn:SetHandler("OnClick", function(_, button) 
        ...
        if settings.maxDurationHealing < 0 then settings.maxDurationHealing = 0 end
        durValuelbl:SetText(tostring(settings.maxDurationHealing)) 
    end)

    -- Max Duration
    local maxDurationlbl = wnd:CreateChildWidget("label", "maxDurationlbl", 0, true)
	maxDurationlbl:SetText("Max duration caused by healing in Ms")
    ...
    maxDurValuelbl:SetText(tostring(settings.maxHealingDurationMs))
    ...
    maxDurIncreaseBtn:SetHandler("OnClick", function(_, button) 
        if api.Input:IsShiftKeyDown() then
            settings.maxHealingDurationMs = (settings.maxHealingDurationMs + 1000)
        else
            settings.maxHealingDurationMs = (settings.maxHealingDurationMs + 100)
        end
        maxDurValuelbl:SetText(tostring(settings.maxHealingDurationMs)) 
    end)
    ...
    maxDurDecreaseBtn:SetHandler("OnClick", function(_, button) 
        ...
        if settings.maxHealingDurationMs < 0 then settings.maxHealingDurationMs = 0 end
        maxDurValuelbl:SetText(tostring(settings.maxHealingDurationMs)) 
    end)
	...
	modOutlbl:SetText("Outgoing Healing Multiplier")
	...
	modInlbl:SetText("Incoming Healing Multiplier")
